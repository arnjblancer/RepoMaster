...
def get_api_config():
    return {
        ...
        "deepseek": {
            "config_list": [
                {"model": "deepseek-v3", "api_key": os.environ.get("DEEPSEEK_API_KEY")}
            ],
        },
        "claude": {
            "config_list": [
...
def validate_and_get_fallback_config(api_type: str = "basic", service_type: str = ""):
    ...
    if not primary_api_key or primary_api_key.strip() == "":
        warnings.warn(
            f"⚠️  API key for '{config_source}' is None or empty. Searching for alternative configurations..."
        )
        ...
        for config_name, config_data in get_api_config().items():
            ...
            if alt_api_key and alt_api_key.strip() != "":
                warnings.warn(
                    f"✅ Found valid API key in '{config_name}' configuration. Using as fallback."
                )
        ...
        for service_name, service_config_data in service_config.items():
            ...
            if service_alt_api_key and service_alt_api_key.strip() != "":
                warnings.warn(
                    f"✅ Found valid API key in service '{service_name}' configuration. Using as fallback."
                )
...
def get_llm_config(
    ...
    # Some models only accept the default temperature. Only include the
    # parameter when it differs from the default to prevent API errors.
