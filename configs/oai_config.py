...
DEFAULT_PROVIDER_PRIORITY = ["openai", "claude", "deepseek", "basic", "azure_openai"]
...
def validate_and_get_fallback_config(api_type: str = None, service_type: str = ""):
    ...
    if not primary_api_key or primary_api_key.strip() == "":
        warnings.warn(
            f"⚠️  API key for '{config_source}' is None or empty. Using provider priority fallback..."
        )
        ...
        try:
            provider, config = get_provider_by_priority()
            warnings.warn(
                f"✅ Using fallback provider '{provider}' with valid configuration"
            )
            return provider, config
        except ValueError:
            raise ValueError(
...
def get_llm_config(
    ...
    # Some models (e.g. reasoning models like o1) only support the default
    # temperature value. To avoid API errors, only include the temperature
